...
modele predykcyjne dla całego szpitala przewidujące liczbę pacjentów na 3, 10 i 30 dni.

"""


def predict3(train_df2, test_df):
    """
    About method predict3: metoda pzrewidująca liczbę pacjentów dla szpitala na 3 dni
    ...
    ax.set_title('Porównanie modelu', fontsize=20)
...
def predict10(train_df2, test_df):
    """
    About method predict10: metoda pzrewidująca liczbę pacjentów dla szpitala na 10 dni
    :param train_df2: zestaw danych treningowych
    :param test_df: zestaw dat, dla których model ma przewidzieć liczbę pacjentów
    :return: pred - tabela wyników
    """
...
def predict30(train_df2, test_df):
    """
    About method predict30: metoda pzrewidująca liczbę pacjentów dla szpitala na 30 dni
    :param train_df2: zestaw danych treningowych
    :param test_df: zestaw dat, dla których model ma przewidzieć liczbę pacjentów
    :return: pred - tabela wyników
    """
    ...
    for index, row in X_test.iterrows():
        ...
        if i in {9, 12, 17}:
            prediction = np.round(lr_model.predict(X_test_one_day))
        elif i in {1, 3, 5, 6, 13, 15, 19, 20, 22, 23, 24, 27, 28, 30}:
            prediction = np.round(dt_model.predict(X_test_one_day))
        else: #2, 4, 7, 8, 10, 11, 14, 16, 18, 2, 25, 26, 29
            prediction = np.round(nn_model.predict(X_test_one_day))
