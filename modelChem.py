...
modele predykcyjne dla oddziału Chemioterapia przewidujące liczbę pacjentów na 3, 10 i 30 dni.

"""



def predictChem3(train_df2, test_df):
    """
    About method predictChem3: metoda pzrewidująca liczbę pacjentów dla oddziału Chemioterapia na 3 dni
    ...
    # Ustawienie etykiet osi X tylko dla dostępnych dat
    ax.set_xticks(y_test.index)

    ax.set_title('Chemioterapia Porównanie modelu', fontsize=20)
...
def predictChem10(train_df2, test_df):
    """
    About method predictChem10: metoda pzrewidująca liczbę pacjentów dla oddziału Chemioterapia na 10 dni
    :param train_df2: zestaw danych treningowych
    :param test_df: zestaw dat, dla których model ma przewidzieć liczbę pacjentów
    :return: pred - tabela wyników
    """
...
def predictChem30(train_df2, test_df):
    """
    About method predictChem30: metoda pzrewidująca liczbę pacjentów dla oddziału Chemioterapia na 30 dni
    :param train_df2: zestaw danych treningowych
    :param test_df: zestaw dat, dla których model ma przewidzieć liczbę pacjentów
    :return: pred - tabela wyników
    """
    ...
    for index, row in X_test.iterrows():
        ...
        # Przewidywanie wartości dla jednego dnia
        if i in {5, 18, 22, 26, 28}:
            nn_model.fit(X_train, y_train)
            prediction = np.round(nn_model.predict(X_test_one_day)) #sieci
        elif i in {1, 13, 17, 20, 24}:
            dt_model.fit(X_train, y_train)
            prediction = np.round(dt_model.predict(X_test_one_day))
        else: #2, 3, 4, 6, 7, 8, 9, 10, 11, 12, 14, 15, 16, 19, 21 23, 25, 27, 29, 30
            lr_model.fit(X_train, y_train)
            prediction = np.round(lr_model.predict(X_test_one_day)) #regresja
